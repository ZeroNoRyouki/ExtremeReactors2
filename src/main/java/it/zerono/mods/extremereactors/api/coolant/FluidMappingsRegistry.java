...
public final class FluidMappingsRegistry {
    ...
    public static Map<Coolant, List<IMapping<Coolant, ITag.INamedTag<Fluid>>>> getCoolantToFluidMap() {
        return unmodifiableInverseMap(s_coolantToFluid);
    }

    public static Map<Vapor, List<IMapping<Vapor, ITag.INamedTag<Fluid>>>> getVaporToFluidMap() {
    ...
    private static <T> Map<T, List<IMapping<T, ITag.INamedTag<Fluid>>>> unmodifiableInverseMap(final Map<T, List<IMapping<T, ITag.INamedTag<Fluid>>>> map) {

        final Object2ObjectMap<T, List<IMapping<T, ITag.INamedTag<Fluid>>>> copy = new Object2ObjectArrayMap<>(map.size());

        for (final Map.Entry<T, List<IMapping<T, ITag.INamedTag<Fluid>>>> entry : map.entrySet()) {
