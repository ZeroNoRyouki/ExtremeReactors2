...
public class ReactorFluidAccessPortContainer
    ...
    private ReactorFluidAccessPortContainer(boolean isClientSide, final int windowId, final Inventory playerInventory,
                                            final ReactorFluidAccessPortEntity port) {

        super(isClientSide, 5, ContainerFactory.EMPTY, Content.ContainerTypes.REACTOR_FLUID_ACCESSPORT.get(), windowId, playerInventory, port);

        final MultiblockReactor reactor = port.getMultiblockController().orElseThrow(IllegalStateException::new);

        this.ACTIVE = BooleanData.of(this, reactor::isMachineActive);
        this.DIRECTION = EnumData.of(this, IoDirection.class, port::getIoDirection);
