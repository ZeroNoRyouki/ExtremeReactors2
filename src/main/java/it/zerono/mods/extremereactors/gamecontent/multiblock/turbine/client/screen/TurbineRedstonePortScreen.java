...
public class TurbineRedstonePortScreen
        extends AbstractRedstonePortScreen<MultiblockTurbine, IMultiblockTurbineVariant, TurbineRedstonePortEntity, TurbineRedstonePortContainer,
    ...
    private void configureFlowRegulatorInput(SensorBehavior behavior, TextInput input) {
        ...
        input.addConstraint(text -> {
            ...
            if (text.isEmpty()) {
                ...
                final long value = MathHelper.clamp(originalValue, 0,
                        this.getTileEntity().getMultiblockVariant().map(v -> (int)v.getMaxRotorSpeed()).orElse(0));
